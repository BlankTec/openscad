...
void MainWindow::updateCamera(const FileContext &ctx)
{
  ...
  if (vpr->getVec3(x, y, z, 0.0)) qglview->cam.setVpr(x, y, z);

  const auto vpt = ctx.lookup_variable("$vpt");
  if (vpt->getVec3(x, y, z, 0.0)) qglview->cam.setVpt(x, y, z);
...
void MainWindow::consoleOutput(const QString &msg)
{
  ...
  if (msg.startsWith("WARNING:") || msg.startsWith("DEPRECATED:")) {
    ...
    QString qmsg = msg;
    if (qmsg.contains('\t') && !qmsg.contains("<pre>", Qt::CaseInsensitive)) this->console->appendPlainText(qmsg);
    else {
      qmsg.replace("\n", "<br>");
